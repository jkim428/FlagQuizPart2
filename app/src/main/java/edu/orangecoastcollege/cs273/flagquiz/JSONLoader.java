...
public class JSONLoader {
    ...
    public static List<Country> loadJSONFromAsset(Context context) throws IOException {
        ...
        try {
            ...
            // COMPLETED: Loop through all the countries in the JSON data, create a Country
            // COMPLETED: object for each and add the object to the allCountriesList

            int length = allCountriesJSON.length();
            for (int i = 0; i < length; i++)
            {
                JSONObject countryObject = allCountriesJSON.getJSONObject(i);
